...
export default function TeamActivityPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // 멤버 활동 데이터 변환
        const memberActivitiesList: MemberActivity[] =
          activitiesData.memberActivities.map((activity) => {
        ...
        const activityLogsList: ActivityLogItem[] =
          activitiesData.recentTeamActivities.map((log) => ({
  ...
  useEffect(() => {
    const loadFilteredLoops = async () => {
      ...
      try {
        ...
        const loops = await fetchTeamLoops(
          Number(teamId),
          undefined,
          statusFilter,
        );
  ...
  // 팀원 활동 정보 (현재 사용자 제외)
  const teamMemberActivities = memberActivities.filter(
    (activity) => !activity.isMe,
  );
      ...
      <main className="flex-1 px-4 py-6 overflow-y-auto">
        ...
        <section className="mt-4 mb-10">
          <div className="flex flex-col gap-2">
            {isLoadingLoops ? (
              ...
              <div className="flex items-center justify-center py-8">
                {}
                <p className="text-body-2-m text-gray-500">
                  해당 상태의 루프가 없습니다
                </p>
              </div>
            ) : (
              filteredLoops.slice(0, 2).map((loop) => {
                ...
                const status =
                  loop.teamStatus === "COMPLETED"
                    ? "완료됨"
                    : loop.teamStatus === "IN_PROGRESS"
                      ? "진행중"
                      : "시작전";
                ...
                return (
                  <div
                    key={loop.id}
                    className="flex items-center justify-between p-4 rounded-[10px] bg-gray-white"
                  >
                    <div className="flex flex-col gap-2 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`text-caption-m px-2 py-0.5 rounded-full ${getStatusColor(status)}`}
                        >
                          {status}
                        </span>
                      </div>
                      ...
                      <p className="text-body-2-m text-gray-500 flex items-center gap-2">
                        ...
                        <span>
                          {loop.repeatCycle || ""} | {formatLoopType(loop.type)}
                        </span>
                      </p>
                    </div>
        ...
        {myActivity && (
          <section className="mb-10">
            {}
            <h2 className="text-body-1-sb text-gray-800">내 활동</h2>
            {}
            <div className="flex items-start justify-between gap-4 p-4 mt-4 rounded-[10px] bg-gray-white">
              <div className="flex flex-col gap-2 flex-1">
                <div className="flex items-start gap-[6px]">
                  {}
                  <span className="items-center justify-center px-[7px] py-[5px] gap-[10px] rounded-[30px] bg-gray-400 text-caption-10-m text-gray-white">
                    시작전·{myActivity.notStartedCount}
                  </span>
                  {}
                  <span className="items-center justify-center px-[7px] py-[5px] gap-[10px] rounded-[30px] bg-primary-500 text-caption-10-m text-gray-white">
                    진행중·{myActivity.inProgressCount}
                  </span>
                  {/* eslint-disable-next-line no-restricted-syntax */}
                  <span className="items-center justify-center px-[7px] py-[5px] gap-[10px] rounded-[30px] bg-[#E1FF9B] text-caption-10-m text-gray-600">
                    완료됨·{myActivity.completedCount}
                  </span>
                </div>
                {}
                <p className="text-body-1-sb text-gray-black">
                  공동 루프 {myActivity.commonLoopCount}개·개별 루프{" "}
                  {myActivity.individualLoopCount}개
                </p>
              ...
              <div className="relative flex h-[90px] w-[90px] items-center justify-center">
                <svg className="h-[90px] w-[90px]" viewBox="0 0 90 90">
                  ...
                  <circle
                    ...
                    strokeDashoffset={
                      2 * Math.PI * 39 -
                      (myActivity.progress / 100) * 2 * Math.PI * 39
                    }
                    strokeLinecap="round"
                ...
                {}
                <span className="absolute text-body-1-b text-gray-black">
                  {myActivity.progress}%
                </span>
              </div>
            </div>
        ...
        {teamMemberActivities.length > 0 && (
          <section className="mb-10">
            {}
            <h2 className="text-body-1-sb text-gray-800">팀원 활동</h2>
            <div className="flex flex-col gap-4 mt-4">
              {teamMemberActivities.map((member) => (
                <div
                  key={member.memberId}
                  className="py-4 px-3 rounded-[10px] bg-gray-white"
                >
                    ...
                    <div className="relative flex h-[90px] w-[90px] items-center justify-center">
                      <svg className="h-[90px] w-[90px]" viewBox="0 0 90 90">
                        ...
                        <circle
                          ...
                          strokeDashoffset={
                            2 * Math.PI * 39 -
                            (member.progress / 100) * 2 * Math.PI * 39
                          }
                          strokeLinecap="round"
                          transform="rotate(-90 45 45)"
                        />
        ...
        <section>
          {}
          <h2 className="text-body-1-sb text-gray-800">팀원 활동 로그</h2>
          {activityLogs.length > 0 ? (
            <div className="flex flex-col gap-2 mt-4">
              {activityLogs.map((log, index) => (
                <div key={index} className="p-4 rounded-[10px] bg-gray-white">
                  {}
                  <p className="text-body-2-m text-gray-800">
                    {log.nickname}님이 &quot;{log.targetName}&quot; 루프를
                    완성했습니다
                  </p>
                  {}
                  <p className="text-body-2-m text-gray-500 mt-1">
