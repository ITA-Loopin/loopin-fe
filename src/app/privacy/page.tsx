...
export default function PrivacyPage() {
  return (
    <div className="overflow-y-auto px-5 py-8">
      <h1 className="text-title-1-b text-[var(--gray-black)] mb-2">
        개인정보처리방침
      </h1>
      <p className="text-caption-m text-[var(--gray-500)] mb-6">
        시행일: 2026년 5월 3일
      </p>

      <p className="text-body-2-m text-[var(--gray-700)] mb-8 leading-relaxed">
      ...
      <Section title="제1조. 수집하는 개인정보 항목 및 수집 방법">
        ...
        <Label>필수 항목</Label>
        <ul className="list-disc pl-5 mb-3 space-y-1 text-body-2-m text-[var(--gray-700)]">
          <li>
        ...
        <Label>선택 항목</Label>
        <ul className="list-disc pl-5 mb-4 space-y-1 text-body-2-m text-[var(--gray-700)]">
          <li>전화번호, 성별, 생년월일</li>
        </ul>

        <SubSection title="2) 서비스 이용 과정에서 자동 수집" />
        <ul className="list-disc pl-5 space-y-1 text-body-2-m text-[var(--gray-700)]">
      ...
      <Section title="제2조. 개인정보의 수집 및 이용 목적">
        <p className="text-body-2-m text-[var(--gray-700)] mb-3">
      ...
      <Section title="제6조. 이용자의 권리와 행사 방법">
        ...
        <p className="text-body-2-m text-[var(--gray-700)]">
      ...
      <Section title="제8조. 개인정보 보호 담당자">
        ...
        />
        <p className="text-body-2-m text-[var(--gray-700)] mt-3">
      ...
      <p className="text-body-2-sb text-[var(--gray-800)] mt-8 mb-12 text-center">
...
function Section({
  ...
  return (
    <section className="mb-8">
      <h2 className="text-body-1-b text-[var(--gray-black)] mb-3">{title}</h2>
      {children}
    </section>
  );
}
...
function SubSection({ title }: { title: string }) {
  return (
    <h3 className="text-body-2-sb text-[var(--gray-800)] mb-2">{title}</h3>
  );
}

function Label({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-body-2-sb text-[var(--gray-600)] mb-1">{children}</p>
  );
}
...
function Table({
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-[var(--gray-300)]">
      <table className="w-full text-body-2-m text-[var(--gray-700)]">
        <thead>
          <tr className="bg-[var(--gray-100)]">
            {headers.map((h) => (
              <th
                key={h}
                className="px-3 py-2 text-left text-body-2-sb text-[var(--gray-800)] border-b border-[var(--gray-300)]"
              >
                {h}
              </th>
            ))}
